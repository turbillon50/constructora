    ...
    <div className="space-y-2">
      {(typeof currentCount === "number" && typeof maxCount === "number") && (
        <div className="flex items-center justify-between text-[11px] text-muted-foreground">
          ...
          {atLimit && (
            <span className="text-neutral-900 font-semibold">Límite alcanzado</span>
          )}
        </div>
      )}
      ...
      <div className="flex gap-2">
        <button
          ...
          onClick={pickCamera}
          disabled={isDisabled}
          className={`${baseBtn} border-neutral-300/60 hover:bg-neutral-50`}
        >
          📷 Cámara
        </button>
        <button
      ...
      {busy && (
        <p className="text-[11px] text-neutral-900 text-center font-semibold animate-pulse">
          {busyLabel || "Procesando..."}
        </p>
      )}

      {!busy && atLimit && (
        <p className="text-[11px] text-neutral-900 text-center">
